...
const SubmitDefence = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:5000/submit_defence', {
                method: 'POST',
                body: formData,
            });
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md pt-10 mt-24 mb-24">
            <h2 className="text-2xl font-extrabold mb-8 text-center text-purple-600"> Defence</h2>
            {existingTeam && existingTeam.pre_defense_status === 'Accepted' ? (
                <div>
                    {existingTeam.defense_status === "Pending" ?
                        (
                    ...
                    <p className='mb-2'><strong>Pre Defence Feedback:</strong> {existingTeam.pre_defense_feedback_sup}</p>


                    <p className=''><strong>Defence Report : </strong>
                        {
                            existingTeam.defence === null || existingTeam.defense_status === "Rejected" ? (
                                <form onSubmit={handleSubmit}>
                                    <div>
                                        <label htmlFor="pdfFile" className="block text-gray-700 font-bold">
                                            Upload Defence Report (PDF)
                                        </label>
                                        <input
                                            type="file"
                                            id="pdfFile"
                ...
                <p className="text-center text-red-500 font-bold">
                    {existingTeam.pre_defense_status !== 'Accepted'
                        ? "Your Pre Defence hasn't been accepted yet."
                        : 'Please submit your Pre Defence Report first.'}
                </p>
            )}
        </div>
    );
